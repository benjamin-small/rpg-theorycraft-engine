...
export const lessons: Lesson[] = [
  ...
  },
  {
    ...
    gamerSummary: 'Games do not always stack bonuses the same way: some percentages add together, while others multiply afterward. This lesson combines those bonus types and recalculates the objective—the number you would expect on a tooltip or DPS sheet. There is still no rotation, resource spending, or cooldown timing.',
    insight: 'A modifier bucket defines which bonuses add together and which apply as separate multipliers.',
    command: `rtce evaluate ${CALC_ARGS}`,
  ...
  },
  {
    ...
    gamerSummary: 'A character sheet usually shows expected damage, not whether your next arrow will crit. This calculation blends normal hits and critical hits by their chances to produce one average objective. It cannot have a lucky streak or a dry spell because it is calculating the long-run average, not rolling individual attacks.',
    insight: 'Crit is described as game data, so different games can define their own crit chance and crit multiplier rules.',
