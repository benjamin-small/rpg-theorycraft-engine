...
//! at `max_stacks` — moves only expiries, and the fold's own inputs
//! (contributions, conditions, tick rate) are all count-driven, so it
//! skips the transaction entirely. ONE caveat, inherited unchanged from
